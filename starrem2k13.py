...
def initialize():
  global G2
  G2 = model.Generator()
  # G2.load_weights("/home/nagyg/Projects/LIDT/Codes/uscope_imgproc/starrem2k13/weights/weights")
  G2.load_weights(os.path.join(os.path.dirname(__file__), "weights", "weights"))
...
def process_channel(channel,pad_width, input_image_size):
    # global progress_bar, current_progress
...
def process_image(source_image):
  ...
  if mode == 'L':
    ...
    for channel in channels[0:3]:
      channel_output = process_channel(channel, pad_width, input_image.size, step_size)
    ...
    raise ValueError(f"Invalid mode for input image: {mode}. Only grayscale(L) and RGB(RGBA) images are supported.")
...
def main():
  ...
  output = process_image(source_image)
  output.save(args[2])
